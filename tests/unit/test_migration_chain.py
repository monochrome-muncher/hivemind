...
1. **Applied migrations are immutable.** yoyo records what it applied by
   id, not by content: editing an already-applied file changes what a
   *fresh* pool gets while every existing pool keeps the old shape —
   the same silent fresh-vs-upgraded divergence that killed the
   declarative re-apply (ADR 0020). The committed checksum file pins
   each shipped migration; changing one is a CI failure, and the fix is
   a NEW migration.
...
class TestMigrationsAreImmutable:
    """A shipped migration is frozen. Corrections are new migrations."""

    def test_every_migration_is_pinned(self) -> None:
        pins = _pinned()
        unpinned = [p.name for p in _migration_files() if p.name not in pins]
        assert not unpinned, (
            f"new migration(s) {unpinned} are not pinned. Add their sha256 to "
            f"{CHECKSUMS.name} in the same commit that introduces them."
        )

    def test_no_pinned_migration_has_changed(self) -> None:
        pins = _pinned()
        changed = [
            p.name for p in _migration_files() if p.name in pins and _digest(p) != pins[p.name]
        ]
        assert not changed, (
            f"applied migration(s) {changed} were edited. yoyo tracks migrations by "
            "id, not content, so an edit changes what a FRESH pool gets while every "
            "existing pool keeps the old shape. Revert the edit and write a NEW "
            "migration instead (ADR 0020)."
        )
    ...
    def test_pins_do_not_reference_deleted_migrations(self) -> None:
        ...
        assert not orphans, (
            f"pinned migration(s) {orphans} no longer exist. A shipped migration is "
            "never deleted — pools that applied it would silently diverge."
        )
